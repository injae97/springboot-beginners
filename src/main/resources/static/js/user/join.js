...
/* 회원가입시 빈칸 없이 제대로 입력했는지 체크하는 메소드 */ 
...
/* 회원가입시 2가지의 비밀번호를 정확하게 입력했는지 체크 */ 
function pwdCheck() {
	const password1 = $(".password1").val().replaceAll(" ", "");
	const password2 = $(".password2").val().replaceAll(" ", "");
	const msgBox = $(".password2").siblings(".msg_box"); // 지정 요소의 모든 형제 요소 (= 동일 부모 요소 갖는 요소) 반환
...
/* ID에 focusout 발생 (focus가 빠져나왔을 때 이벤트 발생) */
